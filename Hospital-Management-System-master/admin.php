<html>
    <head>
        <title>Admin Module</title>
        <link rel="stylesheet" type="text/css" href="Styles.css">
        <link rel="preconnect" href="https://fonts.googleapis.com">
        <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
        <link href="https://fonts.googleapis.com/css2?family=Quicksand:wght@300..700&display=swap" rel="stylesheet">
    </head>
    <body>
        <div id="section2" style="background: linear-gradient(90deg, rgba(2, 0, 36, 1) 0%, rgba(9, 9, 121, 1) 35%, rgba(0, 212, 255, 1) 100%);">
        <h1 style="padding-top:100px">ADMIN</h1>
        <img src="Assets/profile_icon.jpg" height="150px" width="150px" >
        <table cellspacing="15px" cellpadding="15px" style="text-align:center">
            <tr style="background-color: rgba(255,255,255,0.5)">
                <td><a href="add_emp.php">Add Employee</a></td>
                <td><a href="alter_emp.php">Alter Employee Details</a></td>
                <td><a href="get_patient_details.php">Get Patient Details</a></td>
                <td><a href="salary.php">Salary</a></td>
                <td><a href="get_appointment_details.php">Set Appointment Details</a></td>
                <td><a href="get_admission_details.php">Set Admission Details</a></td>
                <td><a href="paybill_completed.php">Get Payment and Generate Bill</a></td>
            </tr>
        </table>
        </div>
    </body>
</html>